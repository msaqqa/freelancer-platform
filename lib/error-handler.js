...
export const handleApiError = (error, options = { showNotification: true }) => {
  ...
  if (!error.response) {
    if (error.code === 'ECONNABORTED') {
      errorResponse = {
        ...
        message: t(DEFAULT_MESSAGES[ERROR_CODES.TIMEOUT_ERROR]),
      };
    } else {
      errorResponse = {
        ...errorResponse,
        status: ERROR_CODES.NETWORK_ERROR,
        message: t(DEFAULT_MESSAGES[ERROR_CODES.NETWORK_ERROR]),
      };
    }
  }
  ...
  switch (errorResponse.status) {
    case ERROR_CODES.UNAUTHORIZED:
      // Do NOT hard-redirect here. Auth is governed by Supabase + the protected
      // layouts; a stray 401 from a legacy Taqat call must not trigger a full
      // page reload to /signin (that showed up as a flash mid-transition).
      break;
    ...
    case ERROR_CODES.FORBIDDEN:
      errorResponse = {
        ...
        message:
          errorResponse.message || t(DEFAULT_MESSAGES[ERROR_CODES.FORBIDDEN]),
      };
      break;
    ...
    default:
      ...
      if (errorResponse.status >= 500) {
        ...
        errorResponse = {
          ...
          message: t(DEFAULT_MESSAGES[ERROR_CODES.DEFAULT]),
        };
      }
  }
